...
class SampleDataset(Dataset):
    def __init__(self, features, contexts, labels, utility_targets, action_targets):
        self.features = torch.from_numpy(features.astype(np.float32))
        self.contexts = torch.from_numpy(contexts.astype(np.float32))
        self.labels = torch.from_numpy(labels.astype(np.float32))
        self.utility_targets = torch.from_numpy(utility_targets.astype(np.float32))
        self.action_targets = torch.from_numpy(action_targets.astype(np.float32))
...
class LearnedMoE(nn.Module):
    def __init__(self, feature_dim: int, context_dim: int, config: TrainConfig):
        ...
        self.trunk = nn.Sequential(
            ...
            nn.Linear(config.hidden_dim, config.trunk_dim),
            nn.GELU(),
            nn.Dropout(config.dropout),
        )
    ...
    def _topk_gate(self, gate_logits):
    ...
    def forward(self, features, contexts):
        trunk_state = self.trunk(features)
...
def load_dataset(path: Path) -> pd.DataFrame:
    ...
    numeric_columns = [
        ...
        if col not in {"timestamp", "session_date_utc", "setup_label"}
    ]
...
def fill_and_standardize(
    ...
    means = feature_frame.loc[train_mask].mean(axis=0)
    stds = feature_frame.loc[train_mask].std(axis=0).replace(0.0, 1.0)
    feature_frame = feature_frame.fillna(means)
    ...
    return feature_frame.to_numpy(np.float32), context_frame.to_numpy(np.float32)


def build_utility_targets(frame: pd.DataFrame, config: TrainConfig) -> Tuple[np.ndarray, np.ndarray]:
    atr = np.nan_to_num(frame["atr_14"].to_numpy(np.float32), nan=1.0, posinf=1.0, neginf=1.0)
...
def subsample_train_indices(frame: pd.DataFrame, train_mask: np.ndarray, config: TrainConfig) -> np.ndarray:
    eligible = train_mask & frame["model_sample_eligible"].to_numpy()
    labels = frame[LABEL_COLUMNS].to_numpy(bool)
    positive_mask = eligible & labels.any(axis=1)
    negative_mask = eligible & ~labels.any(axis=1)
    ...
    sample_indices = np.sort(np.concatenate([positive_indices, negative_indices]))
...
def make_dataset_slices(
    ...
    return SampleDataset(
        ...
        utility_targets=utility_targets[indices],
...
def mean_roc_auc(labels: np.ndarray, probs: np.ndarray) -> float:
    scores = []
    for i in range(labels.shape[1]):
        if np.sum(labels[:, i]) == 0 or np.sum(labels[:, i]) == len(labels[:, i]):
            continue
...
def train_epoch(
    ...
    for features, contexts, labels, utility_targets, action_targets in loader:
...
@torch.no_grad()
def predict(model: LearnedMoE, loader: DataLoader, device: torch.device) -> Dict[str, np.ndarray]:
    model.eval()
    outputs = {
        "expert_probs": [],
        "expert_utilities": [],
    ...
    labels = []
    for features, contexts, batch_labels, utility_targets, action_targets in loader:
        features = features.to(device)
        contexts = contexts.to(device)
        pred = model(features, contexts)
...
def effective_risk_pct(
    ...
    signal_strength = 1.0 - math.exp(-max(normalized_edge, 0.0) / max(config.utility_scale, 1e-6))
    ...
    drawdown_multiplier = min(daily_buffer / max(config.max_daily_loss_pct, 1e-6), 1.0)
    safety_multiplier = (1.0 - config.drawdown_safety_weight) + config.drawdown_safety_weight * drawdown_multiplier
    return max(config.min_risk_pct, min(config.max_risk_pct, risk_pct * urgency_multiplier * safety_multiplier))
...
def backtest_predictions(
    ...
    atr = np.nan_to_num(frame.loc[indices, "atr_14"].to_numpy(np.float32), nan=1.0, posinf=1.0, neginf=1.0)
    stop_distance = np.maximum(config.stop_atr_multiple * atr, 0.25)
    cost_r = config.round_trip_cost_points / stop_distance
    win_r = (config.target_atr_multiple / config.stop_atr_multiple) - cost_r
    loss_r = -1.0 - cost_r
    ...
    for local_pos, row_index in enumerate(indices):
        ...
        if day_realized_pnl_pct[trade_day] <= -config.max_daily_loss_pct:
            ...
            continue
        if balance <= config.starting_balance * (1.0 - config.max_total_drawdown_pct / 100.0):
        ...
        trade_rows.append(
            {
                "setup": setup,
                "gate_weight": float(predictions["gate_weights"][local_pos, chosen_setup_idx[local_pos]]),
    ...
    for balance_value in day_balances:
        ...
        max_drawdown_pct = min(max_drawdown_pct, (balance_value / running_peak - 1.0) * 100.0)
    ...
    return {
        "summary": {
            ...
            "average_hold_bars": round(safe_div(sum(row["bars_held"] for row in trade_rows), len(trade_rows)), 6),
            "ending_balance": round(balance, 2),
            "total_return_pct": round((balance / config.starting_balance - 1.0) * 100.0, 4),
    ...
    for setup_idx, setup in enumerate(SETUPS):
        ...
        out[setup] = {
            "average_precision": round(
                average_precision_score(labels[:, setup_idx], setup_probs),
                6,
            ),
            "roc_auc": round(roc_auc_score(labels[:, setup_idx], setup_probs), 6),
...
def run_train(args) -> int:
    ...
    feature_matrix, context_matrix = fill_and_standardize(frame, masks["train"])
    ...
    val_indices = np.flatnonzero(masks["val"] & frame["model_sample_eligible"].to_numpy())
    test_indices = np.flatnonzero(masks["test"] & frame["model_sample_eligible"].to_numpy())
    ...
    eval_loader_kwargs = {"batch_size": train_config.batch_size, "shuffle": False, "drop_last": False}
    ...
    tuned_policy = tune_prop_policy(frame, val_indices, val_predictions, fixed_risk_config, train_config.seed)
    ...
    save_trades_csv(artifacts_dir / "fixed_risk_backtest_trades.csv.gz", fixed_test_backtest["trades"])
    save_trades_csv(artifacts_dir / "prop_backtest_trades.csv.gz", prop_test_backtest["trades"])
...
def parse_args():
    ...
    parser.add_argument(
        "--artifacts-dir",
        default="artifacts/learned_moe/us100_session_refined",
    )
